...
基础目录: " + AppContext.BaseDirectory, "日志初始化错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
    ...
    class OverlayForm : Form
    {
        ...
        private void UpdateLayeredWindowFromBitmap()
        {
            lock (_bitmapLock)
            {
                ...
                if (!result)
                {
                    ...
                    Logger?.Invoke($"DEBUG: UpdateLayeredWindow failed with error: {errorCode}");
                }
            }
        }
        ...
        {
            // 使用UpdateLayeredWindow，不需要处理传统绘制
        }
    
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            // 使用UpdateLayeredWindow，不需要处理背景绘制
        }
        ...
        private struct Win32Point
        {
            public int X;
            public int Y;
            public Win32Point(int x, int y) { X = x; Y = y; }
        }
        ...
        private struct Win32Size
        {
            public int Width;
            public int Height;
            public Win32Size(int width, int height) { Width = width; Height = height; }
        }
