...
3. 需要以管理员权限运行", ex);
                        ...
                        {
                            throw new InvalidOperationException($"创建桌面复制失败: {ex.Message} (HRESULT: 0x{ex.HResult:X8})", ex);
                        }
                    }
                }
                else
            ...
            try
            {
                ...
                if (DateTime.Now - _lastCaretCheck > TimeSpan.FromMilliseconds(CaretCheckInterval))
                {
                    ...
                    try
                    {
                        ...
                        uint foregroundThread = NativeMethods.GetWindowThreadProcessId(foregroundWindow, IntPtr.Zero);
                        ...
                        {
                            _debugLogger?.Invoke($"[Caret] GetGUIThreadInfo调用失败");
                        }
                    }
                    catch (Exception ex)
                    {
                        _debugLogger?.Invoke($"[Caret] GetGUIThreadInfo调用异常: {ex.Message}");
                    }
                    
                    // 避免透明覆盖层影响检测文本焦点的方法：
                    // 1. 如果当前没有焦点窗口，尝试使用上次记录的焦点窗口
                    if (focusWindow == IntPtr.Zero && _lastFocusWindow != IntPtr.Zero)
                    {
                        _debugLogger?.Invoke("[Caret] 当前无焦点窗口，使用上次记录的焦点窗口");
                        focusWindow = _lastFocusWindow;
                    }
                    
                    // 2. 如果GetGUIThreadInfo提供了有效的光标信息，优先使用
                    if (guiInfoAvailable)
                    {
                        _lastGuiThread = guiThread;
                        _lastGuiCaretPosition = guiCaretPos;
                        _lastCaretPosition = guiCaretPos;
                        _debugLogger?.Invoke($"[Caret] 使用GUI线程信息的光标位置: ({guiCaretPos.X}, {guiCaretPos.Y})");
                    }
                    // 3. 如果GetGUIThreadInfo调用成功但没有有效光标，重置光标位置
                    else if (foregroundWindow != IntPtr.Zero) // 只有当有前台窗口时才考虑重置
                    {
                        _lastCaretPosition = new Point(-1, -1);
                        _debugLogger?.Invoke("[Caret] GUI线程信息中无有效光标，重置光标位置");
                    }
                    // 4. 如果焦点窗口发生变化，更新缓存
                    else if (focusWindow != _lastFocusWindow)
                    {
                        _lastFocusWindow = focusWindow;
                        _lastCaretPosition = new Point(-1, -1); // 重置位置
                        _debugLogger?.Invoke("[Caret] 焦点窗口发生变化，重置光标位置缓存");
                        
                        // 只有当有焦点窗口时才尝试获取光标位置
                        if (focusWindow != IntPtr.Zero)
                        {
                            Point caretPos = new Point();
                            bool result = NativeMethods.GetCaretPos(out caretPos);
                            _debugLogger?.Invoke($"[Caret] 获取文本光标客户区位置: ({caretPos.X}, {caretPos.Y}), 调用结果: {result}");
                            
                            if (result)
                            {
                                // 将光标位置从客户区坐标转换为屏幕坐标
                                Point screenCaretPos = caretPos;
                                bool convertResult = NativeMethods.ClientToScreen(focusWindow, ref screenCaretPos);
                                _debugLogger?.Invoke($"[Caret] 转换为屏幕坐标: ({screenCaretPos.X}, {screenCaretPos.Y}), 转换结果: {convertResult}");
                                _lastCaretPosition = screenCaretPos;
                            }
                            else
                            {
                                _debugLogger?.Invoke("[Caret] 获取文本光标位置失败");
                                _lastCaretPosition = new Point(-1, -1); // 确保重置为无效位置
                            }
                        }
                        else
                        {
                            // 没有焦点窗口时，将光标位置重置
                            _lastCaretPosition = new Point(-1, -1);
                            _debugLogger?.Invoke("[Caret] 没有焦点窗口，重置光标位置");
                        }
                    }
                    else
                    {
                        // 检查当前缓存的位置是否有效，如果无效则不记录日志
                        if (_lastCaretPosition.X == -1 && _lastCaretPosition.Y == -1)
                        {
                            _debugLogger?.Invoke($"[Caret] 当前无有效光标位置，返回无效位置");
                        }
                        else
                        {
                            _debugLogger?.Invoke($"[Caret] 使用缓存的光标位置: ({_lastCaretPosition.X}, {_lastCaretPosition.Y})");
                        }
                    }
                }
                else
                {
                    _debugLogger?.Invoke($"[Caret] 时间间隔太短，使用缓存的光标位置: ({_lastCaretPosition.X}, {_lastCaretPosition.Y})");
                }

                _debugLogger?.Invoke($"[Caret] 返回文本光标位置: ({_lastCaretPosition.X}, {_lastCaretPosition.Y})");
                return _lastCaretPosition;
            }
            catch (Exception ex)
            {
                _debugLogger?.Invoke($"[Caret] 获取文本光标位置异常: {ex.Message}");
                return new Point(-1, -1); // 返回无效位置
            }
        }

        // 获取输入法候选窗口位置
        private Rectangle GetImeCandidateRect()
        {
            try
            {
                // 每ImeCheckInterval毫秒检查一次输入法窗口，避免频繁调用
                if (DateTime.Now - _lastImeCheck > TimeSpan.FromMilliseconds(ImeCheckInterval))
                {
                    ...
                    _debugLogger?.Invoke($"当前活动窗口句柄: {foregroundWindow}");
                    if (foregroundWindow == IntPtr.Zero)
                    {
                        _lastImeRect = Rectangle.Empty;
                        _debugLogger?.Invoke("没有活动窗口");
                        return Rectangle.Empty;
                    }

                    // 获取输入法窗口
                    IntPtr imeWindow = NativeMethods.ImmGetDefaultIMEWnd(foregroundWindow);
                    _debugLogger?.Invoke($"输入法窗口句柄: {imeWindow}");
                    if (imeWindow == IntPtr.Zero)
                    {
                        _lastImeRect = Rectangle.Empty;
                        _debugLogger?.Invoke("没有找到输入法窗口");
                        return Rectangle.Empty;
                    }

                    // 获取候选窗口位置
                    NativeMethods.RECT rect;
                    int result = NativeMethods.SendMessage(imeWindow, NativeMethods.WM_IME_CONTROL, NativeMethods.IMC_GETCANDIDATEPOS, out rect);
                    _debugLogger?.Invoke($"SendMessage结果: {result}, 矩形区域: ({rect.Left}, {rect.Top}, {rect.Right}, {rect.Bottom})");
                    if (result != 0)
                    {
                        _lastImeRect = rect.ToRectangle();
                        _debugLogger?.Invoke($"输入法候选窗口区域: ({_lastImeRect.Left}, {_lastImeRect.Top}, {_lastImeRect.Right}, {_lastImeRect.Bottom})");
                        return _lastImeRect;
                    }
                    else
                    {
                        _debugLogger?.Invoke("获取输入法候选窗口位置失败");
                    }
                }
                else
                {
                    _debugLogger?.Invoke($"使用缓存的输入法窗口区域: ({_lastImeRect.Left}, {_lastImeRect.Top}, {_lastImeRect.Right}, {_lastImeRect.Bottom})");
                }
            ...
            {
                _debugLogger?.Invoke($"获取输入法窗口位置失败: {ex.Message}");
            }
            
            return Rectangle.Empty;
        }

        // 检查指定图块是否在鼠标光标附近、文本光标附近或输入法窗口区域
        private bool IsTileInExcludedArea(int bx, int by)
        {
            // 计算图块的屏幕坐标
            int tileLeft = bx * TileSize;
            int tileTop = by * TileSize;
            int tileRight = Math.Min(tileLeft + TileSize, _screenW);
            int tileBottom = Math.Min(tileTop + TileSize, _screenH);
            Rectangle tileRect = new Rectangle(tileLeft, tileTop, tileRight - tileLeft, tileBottom - tileTop);
            
            // 检查鼠标位置
            Point mousePos = GetMousePosition();
            _debugLogger?.Invoke($"检查鼠标位置: ({mousePos.X}, {mousePos.Y})");
            if (_lastMousePosition != mousePos)
            {
                _lastMousePosition = mousePos;
            }
            
            // 检查是否在鼠标光标附近 (鼠标光标周围MouseExclusionRadiusFactor个图块范围)
            int exclusionRadius = TileSize * MouseExclusionRadiusFactor;
            Rectangle mouseExclusionRect = new Rectangle(
                Math.Max(0, mousePos.X - exclusionRadius),
                Math.Max(0, mousePos.Y - exclusionRadius),
                Math.Min(exclusionRadius * 2, _screenW - Math.Max(0, mousePos.X - exclusionRadius)),
                Math.Min(exclusionRadius * 2, _screenH - Math.Max(0, mousePos.Y - exclusionRadius)));

            if (tileRect.IntersectsWith(mouseExclusionRect))
            {
                _debugLogger?.Invoke($"图块({bx},{by})在鼠标排除区域: 鼠标位置({mousePos.X},{mousePos.Y}), 图块区域({tileLeft},{tileTop},{tileRight},{tileBottom}), 排除区域({mouseExclusionRect.Left},{mouseExclusionRect.Top},{mouseExclusionRect.Right},{mouseExclusionRect.Bottom})");
                return true;
            }
            
            // 检查是否在文本光标附近 (文本光标周围MouseExclusionRadiusFactor个图块范围)
            Point caretPos = GetCaretPosition(); // 获取文本光标位置
            _debugLogger?.Invoke($"检查文本光标位置: ({caretPos.X}, {caretPos.Y})");
            if (caretPos.X >= 0 && caretPos.Y >= 0) // 有效光标位置
            {
                Rectangle caretExclusionRect = new Rectangle(
                    Math.Max(0, caretPos.X - exclusionRadius),
                    Math.Max(0, caretPos.Y - exclusionRadius),
                    Math.Min(exclusionRadius * 2, _screenW - Math.Max(0, caretPos.X - exclusionRadius)),
                    Math.Min(exclusionRadius * 2, _screenH - Math.Max(0, caretPos.Y - exclusionRadius)));
                        
                if (tileRect.IntersectsWith(caretExclusionRect))
                {
                    _debugLogger?.Invoke($"图块({bx},{by})在文本光标排除区域: 光标位置({caretPos.X},{caretPos.Y}), 图块区域({tileLeft},{tileTop},{tileRight},{tileBottom}), 排除区域({caretExclusionRect.Left},{caretExclusionRect.Top},{caretExclusionRect.Right},{caretExclusionRect.Bottom})");
                    return true;
                }
            }
            else
            {
                _debugLogger?.Invoke($"文本光标位置无效: ({caretPos.X}, {caretPos.Y})");
            }
            
            // 检查是否在输入法候选窗口区域
            Rectangle imeRect = GetImeCandidateRect();
            _debugLogger?.Invoke($"检查输入法窗口区域: ({imeRect.Left}, {imeRect.Top}, {imeRect.Right}, {imeRect.Bottom})");
            if (imeRect != Rectangle.Empty && tileRect.IntersectsWith(imeRect))
            {
                _debugLogger?.Invoke($"图块({bx},{by})在输入法排除区域: 输入法区域({imeRect.Left},{imeRect.Top},{imeRect.Right},{imeRect.Bottom}), 图块区域({tileLeft},{tileTop},{tileRight},{tileBottom})");
                return true;
            }
        ...
        private void LogCursorPositionInfo()
        {
            try
            {
                // 获取并记录鼠标位置
                Point mousePos = GetMousePosition();
                _debugLogger?.Invoke($"[持续日志] 鼠标位置: ({mousePos.X}, {mousePos.Y})");
                
                // 获取并记录文本光标位置
                Point caretPos = GetCaretPosition();
                _debugLogger?.Invoke($"[持续日志] 文本光标位置: ({caretPos.X}, {caretPos.Y})");
                
                // 获取并记录输入法候选窗口位置
                Rectangle imeRect = GetImeCandidateRect();
                if (imeRect != Rectangle.Empty)
                {
                    _debugLogger?.Invoke($"[持续日志] 输入法候选窗口区域: ({imeRect.Left}, {imeRect.Top}, {imeRect.Right}, {imeRect.Bottom})");
                }
                else
                {
                    _debugLogger?.Invoke($"[持续日志] 输入法候选窗口区域: 无");
                }
            }
            catch (Exception ex)
            {
                _debugLogger?.Invoke($"[持续日志] 获取光标位置信息时出错: {ex.Message}");
            }
        }
    }
